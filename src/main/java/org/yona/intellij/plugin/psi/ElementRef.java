...
public abstract class ElementRef extends PsiReferenceBase<Identifier> {
  public ElementRef(@NotNull Identifier element) {
...
//			                   ") on "+myElement+" at "+Integer.toHexString(myElement.hashCode()));
  ...
  public boolean isReferenceTo(PsiElement def) {
    ...
    if ( def instanceof Identifier && isDefSubtree(def.getParent()) ) {
      def = def.getParent();
    }
    if ( isDefSubtree(def) ) {
